...
import { type TUserId, UserId } from '@libs/db/ids';
...
export const authRouter = router({
  ...
  listLoginOptions: publicProcedure
    .input(i => parse(EmailSchema, i))
   ...
   */
  withMagicToken: publicProcedure
    .input(i => parse(TokenSchema, i))
    .mutation(async ({ input, ctx }) => {
      const { auth, db } = ctx;
      const { token } = input;
