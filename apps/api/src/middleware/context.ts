import { initDbSdk } from '@libs/db';
import type { TUserId } from '@libs/db/ids';
import { initAuth } from '@libs/trpc';
import { env, getRuntimeKey } from 'hono/adapter';
import { createMiddleware } from 'hono/factory';
import postgres from 'postgres';

import type { HonoEnv, ReqCtx } from '~/types.ts';
...
export const createReqCtx = async (
   ...
   * Auth/session info
   */
  const auth = initAuth({ sql, isDev: vars.ENVIRONMENT === 'development' });
  ...
  let user;
  if (session?.user) {
    const { userId, id, ...rest } = session.user;
    user = { id: id as TUserId, ...rest };
  }
